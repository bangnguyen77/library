class Patron
  ...
  define_method(:checkout) do | attributes |
    ...
    new_checkout = DB.exec("insert into checkout (book_id, patron_id, date, due)
             values (#{book_id.to_i}, #{self.id}, '#{ck_time}', '#{due_time}') returning id;")
    return new_checkout.first().fetch('id').to_i
  end
